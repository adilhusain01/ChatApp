...
const ChatsMessagesScreen = () => {
  ...
  useEffect(() => {
    const fetchRecepientData = async () => {
      try {
        ...
        console.log("Error fetching the recepient data", error);
      }
    };
  ...
  const deleteMessages = async (messagesIds) => {
    try {
      ...
      if (response.ok) {
        setSelectedMessages((previousMessages) =>
          previousMessages.filter((id) => !messagesIds.includes(id))
        );
        fetchMessages();
      } else {
        console.log("Error deleting the messages", response.status);
      }
    } catch (error) {
      console.log("error in deleting the messages", error);
    }
  };
      ...
      >
        {messages.map((item, index) => {
          if (item.messageType === "text") {
            ...
            return (
              <Pressable
                onPress={() => handleSelectedMessage(item)}
                key={index}
                style={[
          ...
          if (item.messageType === "image") {
            const baseURL = "/home/adil/Desktop/build/chat-app/api/files/";
